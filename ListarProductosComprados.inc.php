...
<head>
    ...
    <style>
        ...
        /* .precio {color: #141212; background-color: rgba(253, 251, 251, 0.9);} */
    </style>
</head>
<body>
    <h1 align="center">LISTA DE PRODUCTOS COMPRADOS AL PROVEEDOR</h1>
    <div class="container" >
        <table>
            <thead>
                <tr>
            ...
            <tbody>
                <?php 
                    ...
                    // insertando solo codigo 
                    $sql_compra = "insert into libreria.compras (id_pedido) values ('".$nro_tramite."') ";
                    mysqli_query($con, $sql_compra);
                    $proveedor_compra;
                    $precio_total = 0.0;
                    while($datos = mysqli_fetch_array($consulta)){

                        // neceistamos el nombre
                        $proveedor_compra = $datos['id_proveedor'];
                        $sql_p = "select * from libreria.productos ";
                        $sql_p.= "where id_producto=".$datos['id_prod_prov'];
                        $consulta_p = mysqli_query($con, $sql_p);
                        $datos_p = mysqli_fetch_array($consulta_p);
                        $nombre_producto = $datos_p['nombre'];
                        // echo $nombre_producto;

                        // ahora hay que  ver los precios pero del proveedor
                        $sql_p2 = "select * from libreria.productos_proveedor ";
                        $sql_p2.= "where nombre='".$nombre_producto."' and id_proveedor=".$datos['id_proveedor']." ";
                        $consulta_p2 = mysqli_query($con, $sql_p2);
                        $datos_p2 = mysqli_fetch_array($consulta_p2);
                        
                        echo "<tr>";
                        echo "<td>".$datos['id_prod_prov']." </td>";
                        echo "<td>".$datos_p['nombre']." </td>";
                        echo "<td>".$datos['cantidad']." </td>";
                        echo "<td>".$datos['fecha']." </td>";
                        echo "<td>".$datos_p2['precio_unitario']." Bs.</td>";
                        echo "<td>".$datos_p2['precio_unitario'] * $datos['cantidad']." Bs.</td>";
                        echo "</tr>";
                        $precio_total += $datos_p2['precio_unitario'] * $datos['cantidad'];
                    }
                    echo "<td colspan='5' align ='center' class='precio' > TOTAL PRECIO </td>";
                    ...
                    $sql_compra = "update libreria.compras set id_proveedor='".$proveedor_compra."' , precio_compra='".$precio_total."', fecha='".$fecha_actual_compra."' where id_pedido= '".$nro_tramite."' "; 
                    mysqli_query($con, $sql_compra);
