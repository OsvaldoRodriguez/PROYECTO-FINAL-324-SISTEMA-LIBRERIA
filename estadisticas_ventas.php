...
<head>
    ...
    <style>
        ...
        .buscar{margin-bottom:2%;border-radius: .5em; width: 350px; height: 35px; font-size: 18px; }
        .buscar1{border-radius: .5em; width: 100px; height: 35px; font-size: 18px; }
        .cuadro1{border-radius: .5em;border-radius: .5em; width: 90%; font-size: 18px; border-collapse: collapse; text-align: center; overflow: hidden; box-shadow: 0 0 20px rgba(235, 8, 8, 0.1); background-color: rgba(253, 251, 251, 0.6); }
        table {font-size:23px; margin-top:2%; margin-bottom: 2%; margin-left: 2%; width: 130%; overflow: hidden; box-shadow: 0 0 0 5px rgba(235, 8, 8, 0.1); text-align: center; }
        th, td { padding: 1.5px; background-color: rgba(253, 251, 251, 0.5); color: #141212;}
        thead { background-color: rgba(253, 251, 251, 0.9);}
        .lib{border-radius: .5em; width: 90%; height: 180px; }

        .container { position: absolute; top: 40%; left: 50%; transform: translate(-50%, -50%); }
        .cuadro{margin-left: 5%;border-radius: .5em; border-radius: .5em; width:  auto; font-size: 23px; border-collapse: collapse; text-align: center; overflow: hidden; box-shadow: 0 0 20px rgba(235, 8, 8, 0.1); background-color: rgba(253, 251, 251, 0.6);}
        .anuncio{width:100%;border-radius: .5em;margin-top:5%;   height: 120px; }
        .final{border-radius: .5em;margin-bottom:2%;}
        
        
        /* .div1 { grid-area: 2 / 2 / 9 / 12; }
        .div2 { grid-area: 9 / 3 / 10 / 5; } */
        .div1 { grid-area: 9 / 5 / 9 / 8; }
        .div4 { grid-area: 8 / 6 / 11 / 9; }
        .div5 { grid-area: 10 / 6 / 11 / 9; }
        .bordeado{border-radius: .5em;}
        .boton { 
			    border-radius: .5em;
          font-family: "Century Gothic";
...
<body>
    <h1 align="center">ESTADISTICAS DE VENTAS POR CLIENTE </h1>

    <div class="container">
        <div class="cuadro">
            <!-- <div> <img src="img/estadisticas.jpg" alt="" class="anuncio"> </div> -->
            <div class="final">
                <br>
                En este apartado podras ver los productos vendidos por cliente <br> <br/>
                Seleccione un Cliente
            </div>

            <form method="GET" action="estadisticas_listado_cliente.php">

                <select name="id_producto_comprado" class="buscar">
                    <?php 
                    include "conexion.inc.php";
                    // buscamos que id tiene rol como cliente
                    $sql = "select * from rol where descripcion='Cliente' ";
                    $consulta = mysqli_query($con, $sql);
                    $datos = mysqli_fetch_array($consulta);
                    $id_cliente = $datos['id'];
                    
                    // ahora si buscamos a los clientes
                    $sql = "select * from rolusuario where idRol='".$id_cliente."' ";
                    $consulta = mysqli_query($con, $sql);
                    while($datos = mysqli_fetch_array($consulta)){
                        // ahora encontrando el nombre del cliente
                        // echo $datos['IdUsuario'];
                        $sql_c = "select * from usuario where id='".$datos['IdUsuario']."' ";
                        $consulta_c = mysqli_query($con, $sql_c);
                        $datos_c = mysqli_fetch_array($consulta_c);
                        echo "<option value='".$datos['IdUsuario']."'>".$datos_c['descripcion']."</option>";
    ...
    <div class='parent'>
        ...
        <div class='div4'>
                <button class="boton" type='submit' name='estadisticas_listado_cliente' value = 'estadisticas_listado_cliente'> Listar </button>
        ...
        <div class='div5'>
            <form  method="GET" action="estadisticas.php">
                <button class="boton" type='submit' name='volver' value = 'volver'> Volver </button>
            </form>
        </div>
    </div>
